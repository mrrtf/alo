...
void convert_legacyseg(AliMpDDLStore* ddlStore, AliMpSegmentation* mseg)
{
  std::vector<int> deids = get_deids(ddlStore);
  ...
  all_legacyseg2json(b, nb, OF("legacyseg.json").Writer());
}

void convert_seg(AliMpDDLStore* ddlStore, AliMpSegmentation* mseg)
{
  // [ segname -> [ vector<mppos> b, vector<mppos> nb ] ]
  ...
  std::vector<std::pair<float, float>> padsizes = get_padsizes(ddlStore, mseg);
  std::vector<AliMpVMotif*> motifs = get_allmotifs(pcbs, sectors, padsizes);
  all_seg2json(segnames,motifpositions,motifs,padsizes,OF("segmentation.json").Writer());
}
...
int main(int argc, char* argv[])
{
  ...
  convert_legacyseg(ddlStore, mseg);
