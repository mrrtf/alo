...
ClassImp(AliHLTMUONDigitLoaderComponent)
    ...
    namespace {
        ...
        // we are not using AliHLTCDHWrapper as that include file
        // is not installed by AliRoot, so unreachable to us...
    
    AliHLTUInt32_t GetHeaderSize(void* ptr) {
       return (reinterpret_cast<const AliRawDataHeader*>(ptr)->GetVersion()==2 ? 
...
void AliHLTMUONDigitLoaderComponent::GetOutputDataSize(unsigned long& constBase, double& inputMultiplier)
{
  ...
  constBase = sizeof(AliHLTMUONDigitBlockStruct) + 1064008*sizeof(AliHLTMUONDigitStruct);
  inputMultiplier = 0;
}
  ...
  AliHLTComponentBlockDataList& outputBlocks )
{
  ...
  while (pBlock)
  {
    ...
    AliHLTUInt32_t headerSize = GetHeaderSize(pBlock->fPtr);
...
int AliHLTMUONDigitLoaderComponent::CreateDigitBlock(
  AliHLTUInt8_t *outputPtr,
  AliHLTUInt32_t size,
  AliHLTMUONDigitBlockWriter *&digitblock ) const
{
