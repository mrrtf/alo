...
//-----------------------------------------------------------------------------
/// \class AliMUONClusterSplitterMLEMdebug
///
...
using std::cout;
using std::endl;
//const Double_t AliMUONClusterSplitterMLEMdebug::fgkCouplMin = 1.e-3; // threshold on coupling
const Double_t AliMUONClusterSplitterMLEMdebug::fgkCouplMin = 1.e-2; // threshold on coupling

//_____________________________________________________________________________
AliMUONClusterSplitterMLEMdebug::AliMUONClusterSplitterMLEMdebug(Int_t detElemId,
                                                                 TObjArray* pixArray,
...
//_____________________________________________________________________________
AliMUONClusterSplitterMLEMdebug::~AliMUONClusterSplitterMLEMdebug()
{
  /// Destructor
...
//_____________________________________________________________________________
void AliMUONClusterSplitterMLEMdebug::AddBin(TH2* mlem,
...
//_____________________________________________________________________________
void AliMUONClusterSplitterMLEMdebug::AddCluster(Int_t ic, Int_t nclust,
                                                 TMatrixD& aijcluclu,
...
//_____________________________________________________________________________
TObject*
  AliMUONClusterSplitterMLEMdebug::BinToPix(TH2* mlem,
                                            Int_t jc, Int_t ic)
{
  ...
  for (Int_t i = 0; i < nPix; ++i) {
    ...
    if (pixPtr->Charge() < fLowestPixelCharge)
      continue;
...
//_____________________________________________________________________________
Float_t
  AliMUONClusterSplitterMLEMdebug::ChargeIntegration(Double_t x, Double_t y,
...
//_____________________________________________________________________________
void AliMUONClusterSplitterMLEMdebug::Fcn1(const AliMUONCluster& cluster,
...
//_____________________________________________________________________________
Double_t AliMUONClusterSplitterMLEMdebug::Param2Coef(Int_t icand, Double_t coef, Double_t* par) const
{
...
//_____________________________________________________________________________
Int_t AliMUONClusterSplitterMLEMdebug::Fit(const AliMUONCluster& cluster,
                                           ...
                                           TObjArray& clusterList, TH2* mlem)
{
  ...
  if (fDebug) {
    ...
    for (Int_t j = 0; j < cluster.Multiplicity(); ++j) {
      ...
      if (pad->Status() == AliMUONClusterFinderMLEM::GetUseForFitFlag() && !pad->IsSaturated())
        npadOK++;
    }
  ...
  if (nfitMax > 1) {
    if (((nInX < 3) && (nInY < 3)) || ((nInX == 3) && (nInY < 3)) || ((nInX < 3) && (nInY == 3)))
      nfitMax = 1; // not enough pads in each direction
  }
  if (nfit > nfitMax)
    nfit = nfitMax;
  ...
  if (fDebug)
    cout << xyCand[0][0] << " " << xyCand[0][1] << " " << sigCand[0][0] << " " << sigCand[0][1] << endl;
  ...
  for (Int_t iseed = 0; iseed < nfit; ++iseed) {
    ...
    // Try new algorithm
    min = nLoop = 1;
    stepMax = func2[1] = derMax = 999999;
    nFail = 0;

    while (1) {
      max = !min;
      Fcn1(cluster, fNpar, gin, func0, param, iflag);
      nCall++;
      iflag = 1;
      ...
      for (Int_t j = 0; j < fNpar; ++j) {
        ...
        param[j] += delta[j] / 10;
        if (j > 0)
          param[j - 1] -= delta[j - 1] / 10;
        Fcn1(cluster, fNpar, gin, func1, param, iflag);
        nCall++;
      ...
      for (Int_t j = 0; j < fNpar; ++j) {
        ...
        if (nLoop == 1) {
          ...
          shift[j] = 0;
        } else if (((deriv[min][j] * deriv[!min][j] > 0) && (TMath::Abs(deriv[min][j]) > TMath::Abs(deriv[!min][j]))) || (TMath::Abs(deriv[0][j] - deriv[1][j]) < 1.e-3) || (TMath::Abs(dder[j]) < 1.e-6)) {
        ...
        // Too big step
        if (TMath::Abs(shift[j]) / step0[j] > 10)
          shift[j] = TMath::Sign(10., shift[j]) * step0[j]; //
      ...
      if (((estim < 1) && (derMax < 2)) || nLoop > 150)
        break; // minimum was found
    ...
    chi2n = fmin / nDof;
    if (fDebug)
      cout << " Chi2 " << chi2n << " " << fNpar << endl;

    //if (fNpar > 2) cout << param0[min][fNpar-3] << " " << chi2n * (1+TMath::Min(1-param0[min][fNpar-3],0.25)) << endl;
    //if (chi2n*1.2+1.e-6 > chi2o )
    if (fNpar > 2 && (chi2n > chi2o || ((iseed == nfit - 1) && (chi2n * (1 + TMath::Min(1 - param0[min][fNpar - 3], 0.25)) > chi2o)))) {
      fNpar -= 3;
      break;
    }
...
//_____________________________________________________________________________
void AliMUONClusterSplitterMLEMdebug::Split(const AliMUONCluster& cluster,
  ...
  */
  ...
  for (Int_t iclust = 0; iclust < nclust; ++iclust) {
    ...
    for (Int_t i = 0; i < npxclu; ++i) {
      ...
      for (Int_t j = 0; j < npad; ++j) {
        ...
        if (coef[j * nPix + indx] < fgkCouplMin)
          continue;
  ...
  for (Int_t iclust = 0; iclust < nclust; ++iclust) {
    for (Int_t j = 0; j < npad; ++j) {
      ...
      if (cluster.Pad(j)->IsSaturated())
        continue;
      if (aijclupad(iclust, j) < fgkCouplMin)
        continue;
      for (Int_t iclust1 = iclust + 1; iclust1 < nclust; iclust1++) {
        if (aijclupad(iclust1, j) < fgkCouplMin)
          continue;
  ...
  for (Int_t igroup = 0; igroup < nclust; ++igroup) {
    ...
    while (nCoupled > 0) {
      if (nCoupled < 4) {
        ...
        if (fDebug)
          cout << " nForFit " << nForFit << " " << coupl << endl;
      } // else
      ...
      if (nCoupled > 3) {
        ...
        for (Int_t j = 0; j < npad; ++j) {
          ...
          if (pad->Status() != AliMUONClusterFinderMLEM::GetModifiedFlag())
            continue;
          for (Int_t iclust = 0; iclust < nCoupled; ++iclust) {
            indx = clustNumb[iclust];
            if (aijclupad(indx, j) < fgkCouplMin)
              continue;
            for (Int_t iclust1 = iclust + 1; iclust1 < nCoupled; ++iclust1) {
              indx1 = clustNumb[iclust1];
              if (aijclupad(indx1, j) < fgkCouplMin)
                continue;
              // Check this
...
//_____________________________________________________________________________
void AliMUONClusterSplitterMLEMdebug::Merge(const AliMUONCluster& cluster,
...
//_____________________________________________________________________________
Double_t
  AliMUONClusterSplitterMLEMdebug::MinGroupCoupl(Int_t nCoupled, const Int_t* clustNumb,
...
//_____________________________________________________________________________
Int_t AliMUONClusterSplitterMLEMdebug::SelectPad(const AliMUONCluster& cluster,
                                                 ...
                                                 const TMatrixD& aijclupad)
{
  ...
  for (Int_t iclust = 0; iclust < nForFit; ++iclust) {
    ...
    for (Int_t j = 0; j < npad; ++j) {
      ...
      */
      if (pad->Status() != AliMUONClusterFinderMLEM::GetZeroFlag() || pad->IsSaturated())
        continue; // used pads and overflows
...
//_____________________________________________________________________________
void AliMUONClusterSplitterMLEMdebug::UpdatePads(const AliMUONCluster& cluster,
                                                 Int_t /*nfit*/, Double_t* par)
{
  ...
  for (Int_t j = 0; j < mult; ++j) {
    ...
    if (pad->Status() != AliMUONClusterFinderMLEM::GetCoupledFlag())
      continue;
    if (fNpar != 0) {
      charge = 0;
      for (Int_t i = 0; i <= iend; ++i) {
    ...
    if (pad->Charge() > fLowestPadCharge)
      pad->SetStatus(AliMUONClusterFinderMLEM::GetZeroFlag());
    // return pad for further using // FIXME: remove usage of zerosuppression here
    else
      pad->SetStatus(AliMUONClusterFinderMLEM::GetOverFlag()); // do not use anymore
