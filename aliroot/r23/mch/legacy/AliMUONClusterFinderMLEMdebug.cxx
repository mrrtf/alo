...
//-----------------------------------------------------------------------------
/// \class AliMUONClusterFinderMLEMdebug
///
...
/// a list of clusters is output (using AliMUONClusterSplitterMLEMdebug).
///
...
#include "AliMUONClusterSplitterMLEMdebug.h"
...
const Double_t AliMUONClusterFinderMLEMdebug::fgkDistancePrecision = 1e-3; // (cm) used to check overlaps and so on
const TVector2 AliMUONClusterFinderMLEMdebug::fgkIncreaseSize(-AliMUONClusterFinderMLEMdebug::fgkDistancePrecision, -AliMUONClusterFinderMLEMdebug::fgkDistancePrecision);
const TVector2 AliMUONClusterFinderMLEMdebug::fgkDecreaseSize(AliMUONClusterFinderMLEMdebug::fgkDistancePrecision, AliMUONClusterFinderMLEMdebug::fgkDistancePrecision);

// Status flags for pads
const Int_t AliMUONClusterFinderMLEMdebug::fgkZero = 0x0;        ///< pad "basic" state
const Int_t AliMUONClusterFinderMLEMdebug::fgkMustKeep = 0x1;    ///< do not kill (for pixels)
const Int_t AliMUONClusterFinderMLEMdebug::fgkUseForFit = 0x10;  ///< should be used for fit
const Int_t AliMUONClusterFinderMLEMdebug::fgkOver = 0x100;      ///< processing is over
const Int_t AliMUONClusterFinderMLEMdebug::fgkModified = 0x1000; ///< modified pad charge
const Int_t AliMUONClusterFinderMLEMdebug::fgkCoupled = 0x10000; ///< coupled pad

//_____________________________________________________________________________
AliMUONClusterFinderMLEMdebug::AliMUONClusterFinderMLEMdebug(Bool_t plot, AliMUONVClusterFinder* clusterFinder)
...
//_____________________________________________________________________________
AliMUONClusterFinderMLEMdebug::~AliMUONClusterFinderMLEMdebug()
{
  /// Destructor
  delete fPixArray;
  fPixArray = 0;
...
//_____________________________________________________________________________
Bool_t
  AliMUONClusterFinderMLEMdebug::Prepare(Int_t detElemId,
                                         TObjArray* pads[2],
                                         ...
                                         const AliMpVSegmentation* seg[2])
{
  ...
  delete fSplitter;
  fSplitter = new AliMUONClusterSplitterMLEMdebug(fDetElemId,
                                                  fPixArray,
                                                  fLowestPixelCharge,
                                                  fLowestPadCharge,
...
//_____________________________________________________________________________
AliMUONCluster*
  AliMUONClusterFinderMLEMdebug::NextCluster()
{
  /// Return next cluster
  //  AliCodeTimerAuto("",0)
...
//_____________________________________________________________________________
Bool_t
  AliMUONClusterFinderMLEMdebug::WorkOnPreCluster()
{
  ...
  if (nInX < 4 && nInY < 4) {
    ...
    if (nMax > 1) {
      if (cluster->Multiplicity() <= 50)
        nMax = 1; // for small clusters
      if (nMax > 1)
        TMath::Sort(nMax, maxVal, maxPos, kTRUE); // in descending order
    }
  }
  ...
  for (Int_t i = 0; i < nMax; ++i) {
    ...
    if (i < nMax - 1) {
      ...
      for (Int_t j = 0; j < mult; ++j) {
        ...
        if (pad->Status() != fgkOver)
          continue; // pad was not used
        //pad->SetStatus(0);
...
//_____________________________________________________________________________
Bool_t
  AliMUONClusterFinderMLEMdebug::Overlap(const AliMUONPad& pad, const AliMUONPad& pixel)
{
...
//_____________________________________________________________________________
AliMUONCluster*
  AliMUONClusterFinderMLEMdebug::CheckPrecluster(const AliMUONCluster& origCluster)
{
...
//_____________________________________________________________________________
AliMUONCluster*
  AliMUONClusterFinderMLEMdebug::CheckPreclusterTwoCathodes(AliMUONCluster* cluster)
{
  ...
  for (Int_t i = 0; i < npad; ++i) {
    ...
    for (Int_t j = i + 1; j < npad; ++j) {
      ...
      if (padj->Cathode() != 1)
        continue;
      if (!AliMUONPad::AreOverlapping(*padi, *padj, fgkDecreaseSize))
        continue;
  ...
  if (nFlags > 0) {
    ...
    for (Int_t i = 0; i < npad; ++i) {
      ...
      if (!mpPad.IsValid())
        continue;
      if (nFlags == 1 && pad->Charge() < fLowestPadCharge)
        continue;
  ...
  if (!cluster->IsSaturated() && cluster->ChargeAsymmetry() > 1) {
    ...
    for (Int_t i = 0; i < mult; ++i) {
      ...
      if (dist[indx] > dmin) {
        ...
        if (dx >= 0 && dy >= 0)
          continue;
        if (TMath::Abs(dx) > TMath::Abs(dy) && dx >= 0)
          continue;
        if (TMath::Abs(dy) > TMath::Abs(dx) && dy >= 0)
          continue;
      }
      if (dist[indx] > distPrev + 1)
        break; // overstepping empty pads
...
//_____________________________________________________________________________
void AliMUONClusterFinderMLEMdebug::CheckOverlaps()
{
...
//_____________________________________________________________________________
void AliMUONClusterFinderMLEMdebug::BuildPixArray(AliMUONCluster& cluster)
{
...
//_____________________________________________________________________________
void AliMUONClusterFinderMLEMdebug::BuildPixArrayOneCathode(AliMUONCluster& cluster)
{
  ...
  Double_t width[2] = { dim.X(), dim.Y() };
  Double_t xy0[2] = { 99999, 99999 };
  ...
  if (cluster.Multiplicity(0) == 0)
    cath0 = 1;
  else if (cluster.Multiplicity(1) == 0)
    cath1 = 0;
  ...
  for (Int_t i = 0; i < 2; ++i) {
    ...
    if (TMath::Abs(dist) < 1.e-6)
      dist = -1.e-6;
    min[i] = xy0[i] + (TMath::Nint(dist - TMath::Sign(1.e-6, dist)) + TMath::Sign(0.5, dist)) * width[i] * 2;
  ...
  for (Int_t i = 1; i <= nbins[0]; ++i) {
    ...
    for (Int_t j = 1; j <= nbins[1]; ++j) {
      if (hist2->GetBinContent(hist2->GetBin(i, j)) < 0.1)
        continue;
      ...
      if (cath0 != cath1) {
        ...
        if (cont < 999.)
          continue;
        if (cont - Int_t(cont / 1000.) * 1000. < 0.5)
          continue;
      }
...
//_____________________________________________________________________________
void AliMUONClusterFinderMLEMdebug::PadOverHist(Int_t idir, Int_t ix0, Int_t iy0, AliMUONPad* pad,
                                                TH2D* hist1, TH2D* hist2)
{
  ...
  for (Int_t i = 0; i < nbinPad; ++i) {
    ...
    if (lowEdge + fgkDistancePrecision > pad->Coord(idir) + pad->Size(idir))
      break;
    if (idir == 0)
      PadOverHist(1, ixy, iy0, pad, hist1, hist2); // span in the other direction
    else {
      // Fill histogram
  ...
  for (Int_t i = -1; i > -nbinPad; --i) {
    ...
    if (upEdge - fgkDistancePrecision < pad->Coord(idir) - pad->Size(idir))
      break;
    if (idir == 0)
      PadOverHist(1, ixy, iy0, pad, hist1, hist2); // span in the other direction
    else {
...
//_____________________________________________________________________________
void AliMUONClusterFinderMLEMdebug::Plot(const char* /*basename*/)
{
...
//_____________________________________________________________________________
void AliMUONClusterFinderMLEMdebug::ComputeCoefficients(AliMUONCluster& cluster,
                                                        Double_t* coef,
                                                        Double_t* probi)
{
  ...
  for (Int_t j = 0; j < npadTot * nPix; ++j)
    coef[j] = 0.;
...
//_____________________________________________________________________________
Bool_t AliMUONClusterFinderMLEMdebug::MainLoop(AliMUONCluster& cluster, Int_t iSimple)
{
  ...
  for (Int_t i = 0; i < npadTot; ++i) {
    ...
    if (cluster.Pad(i)->Status() == fgkZero)
      ++npadOK;
  }
  ...
  while (1) {
    ...
    if (lc == 1) {
      for (Int_t ipix = 1; ipix < nPix; ++ipix) {
        ...
        for (Int_t i = 0; i < 2; ++i) {
          Int_t indx = i * 2;
          if (pixPtr->Coord(i) < xylim[indx])
            xylim[indx] = pixPtr->Coord(i);
          else if (-pixPtr->Coord(i) < xylim[indx + 1])
            xylim[indx + 1] = -pixPtr->Coord(i);
        }
      }
    } else
      pixPtr = Pixel(0);
    ...
    for (Int_t ipix = 0; ipix < nPix1; ++ipix) {
      ...
      for (Int_t i = 0; i < 2; ++i) {
        if (!i) {
          ...
          continue; // skip adjustment of histo limits
        for (Int_t j = 0; j < 4; ++j) {
...
//_____________________________________________________________________________
void AliMUONClusterFinderMLEMdebug::MaskPeaks(Int_t mask)
{
  ...
  for (Int_t i = 0; i < fPixArray->GetEntriesFast(); ++i) {
    ...
    if (pix->Status() == fgkMustKeep) {
      if (mask == 1)
        pix->SetCharge(pix->Charge() + 10000.);
      else
        pix->SetCharge(pix->Charge() - 10000.);
    }
  }
...
//_____________________________________________________________________________
void AliMUONClusterFinderMLEMdebug::Mlem(AliMUONCluster& cluster,
...
//_____________________________________________________________________________
void AliMUONClusterFinderMLEMdebug::FindCOG(Double_t* xyc)
{
  /// Calculate position of the center-of-gravity around the maximum pixel
...
//_____________________________________________________________________________
Int_t AliMUONClusterFinderMLEMdebug::FindNearest(const AliMUONPad* pixPtr0)
{
  ...
  for (Int_t i = 0; i < nPix; ++i) {
    ...
    if (pixPtr == pixPtr0 || pixPtr->Charge() < fLowestPixelCharge)
      continue;
...
//_____________________________________________________________________________
void AliMUONClusterFinderMLEMdebug::Paint(Option_t*)
{
...
//_____________________________________________________________________________
Int_t AliMUONClusterFinderMLEMdebug::FindLocalMaxima(TObjArray* pixArray, Int_t* localMax, Double_t* maxVal)
{
  ...
  Int_t nx = TMath::Nint((-xylim[1] - xylim[0]) / pixPtr->Size(0) / 2);
  Int_t ny = TMath::Nint((-xylim[3] - xylim[2]) / pixPtr->Size(1) / 2);
  if (pixArray == fPixArray)
    fHistAnode = new TH2D("anode", "anode", nx, xylim[0], -xylim[1], ny, xylim[2], -xylim[3]);
  else
    fHistAnode = new TH2D("anode1", "anode1", nx, xylim[0], -xylim[1], ny, xylim[2], -xylim[3]);
  for (Int_t ipix = 0; ipix < nPix; ++ipix) {
    pixPtr = (AliMUONPad*)pixArray->UncheckedAt(ipix);
  ...
  for (Int_t i = 0; i < nxy; ++i)
    isLocalMax[i] = 0;

  for (Int_t i = 1; i <= ny; ++i) {
    indx = (i - 1) * nx;
    for (Int_t j = 1; j <= nx; ++j) {
      if (fHistAnode->GetBinContent(fHistAnode->GetBin(j, i)) < fLowestPixelCharge)
        continue;
...
//_____________________________________________________________________________
void AliMUONClusterFinderMLEMdebug::FlagLocalMax(TH2D* hist, Int_t i, Int_t j, Int_t* isLocalMax)
{
...
//_____________________________________________________________________________
void AliMUONClusterFinderMLEMdebug::FindCluster(AliMUONCluster& cluster,
...
//_____________________________________________________________________________
void AliMUONClusterFinderMLEMdebug::AddBinSimple(TH2D* hist, Int_t ic, Int_t jc)
{
...
//_____________________________________________________________________________
AliMUONClusterFinderMLEMdebug&
  AliMUONClusterFinderMLEMdebug::operator=(const AliMUONClusterFinderMLEMdebug& rhs)
{
...
//_____________________________________________________________________________
void AliMUONClusterFinderMLEMdebug::AddVirtualPad(AliMUONCluster& cluster)
{
  ...
  if (TMath::Abs(dim0.Y() - dim1.Y()) < fgkDistancePrecision)
    same = kTRUE; // the same pad size on both planes
  ...
  for (Int_t inb = 0; inb < 2; ++inb) {
    ...
    if (inb == 0 && AliMp::PairFirst(cn) == 2)
      check[inb] = kTRUE; // check non-bending plane
    else if (inb == 1 && AliMp::PairSecond(cn) == 2)
      check[inb] = kTRUE; // check bending plane
    if (same) {
  ...
  for (Int_t inb = 0; inb < 2; ++inb) {
    ...
    for (Int_t j = 0; j < mult; ++j) {
      ...
      if (pad->Cathode() != nonb[inb])
        continue;
      if (pad->Coord(inb) < limits[0])
        limits[0] = pad->Coord(inb);
      if (pad->Coord(inb) > limits[1])
        limits[1] = pad->Coord(inb);
    }
    ...
    for (Int_t j = 0; j < 2; ++j) {
      if (j == 1) {
        ...
        if (!add)
          break;
        if (amax[1] / amax[0] < 0.5)
          break;
      }
      // Check if pad at the cluster limit
      AliMUONPad* pad = cluster.Pad(maxPads[j]);
      Int_t idir = 0;
      if (TMath::Abs(pad->Coord(inb) - limits[0]) < fgkDistancePrecision)
        idir = -1;
      else if (TMath::Abs(pad->Coord(inb) - limits[1]) < fgkDistancePrecision)
        idir = 1;
      else {
      ...
      if (j == 1 && idir == idirAdd)
        break; // this pad has been already added

      // Add pad (if it exists)
      TVector2 pos;
      if (inb == 0)
        pos.Set(pad->X() + idir * (pad->DX() + fgkDistancePrecision), pad->Y());
      else
        pos.Set(pad->X(), pad->Y() + idir * (pad->DY() + fgkDistancePrecision));
      ...
      if (inb == 0)
        muonPad.SetCharge(TMath::Min(amax[j] / 100, fLowestPadCharge));
      //else muonPad.SetCharge(TMath::Min (amax[j]/15, fgkZeroSuppression));
      else
        muonPad.SetCharge(TMath::Min(amax[j] / 15, fLowestPadCharge));
      if (muonPad.Charge() < 2.0 * fLowestPixelCharge)
        muonPad.SetCharge(2.0 * fLowestPixelCharge);
      muonPad.SetReal(kFALSE);
      if (fDebug)
        printf(" ***** Add virtual pad in %d direction ***** %f %f %f %3d %3d %f %f \n",
...
//_____________________________________________________________________________
void AliMUONClusterFinderMLEMdebug::PadsInXandY(AliMUONCluster& cluster,
...
//_____________________________________________________________________________
void AliMUONClusterFinderMLEMdebug::RemovePixel(Int_t i)
{
...
//_____________________________________________________________________________
void AliMUONClusterFinderMLEMdebug::Simple(AliMUONCluster& cluster)
{
...
//_____________________________________________________________________________
AliMUONPad*
  AliMUONClusterFinderMLEMdebug::Pixel(Int_t i) const
{
...
//_____________________________________________________________________________
void AliMUONClusterFinderMLEMdebug::Print(Option_t* what) const
{
  /// printout
  TString swhat(what);
  swhat.ToLower();
...
//_____________________________________________________________________________
void AliMUONClusterFinderMLEMdebug::SetChargeHints(Double_t lowestPadCharge, Double_t lowestClusterCharge)
{
