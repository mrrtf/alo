...
struct MotifPosition {
 public:
  MotifPosition(int f, int m, int p, double x, double y) : mFECId(f), mMotifTypeId(m), mPadSizeId(p), mPositionX(x), mPositionY(y)
  {
  }

  MotifPosition(int f, int m, int p1, double x, double y,
                int p2, std::vector<int> list) : mFECId(f), mMotifTypeId(m), mPadSizeId(p1), mPositionX(x), mPositionY(y), mSecondPadSizeId(p2), mPadNumbers(list)
  {
  }
...
std::vector<MotifPosition>
  getMotifPositions(int segtype, bool bending, const rapidjson::Value& catsegs, const rapidjson::Value& motiftypes,
